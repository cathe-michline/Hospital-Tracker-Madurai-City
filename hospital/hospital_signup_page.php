...
session_start();
    ...
    $confirmpassword=$_POST['confirmpassword'];
		$check_mail_query = mysqli_query($con,"SELECT hname FROM hadmin WHERE hname='".$hname."'");
    ...
    if(mysqli_num_rows($check_mail_query) != 0){
      echo '<script>alert("Another user registered using this email, Try again!")</script>';
		...
		if(!empty($hname) && !empty($password) && !empty($confirmpassword))
		{
        ...
        if(strcmp($password,$confirmpassword)>0)
          ...
          //save to database
          $query = "insert into hadmin (hname,password) values ('$hname','$password')";
